<template>
  <div class="layout-padding">
    <p class="text-bold">Data Table</p>

    <p>Example</p>
    <q-collapsible label="Showcasing options" sublabel="Change them to see it in action..." style="max-width: 600px; margin-bottom: 25px" class="shadow-2">
      ...
      <q-field icon="widgets" label="Some Features" :label-width="4">
        <div class="column group" style="margin: -5px -7px">
          ...
          <q-checkbox v-model="config.pagination" label="Pagination" />
...
<script>
  import { QDataTable, QField, QInput, QCheckbox, QSelect, QSlider, QBtn, QIcon, QTooltip, QCollapsible, clone, Toast } from 'quasar'

  import table from 'data/table.json'

  export default {
    components: {
      QDataTable, QField, QInput, QCheckbox, QSelect, QSlider, QBtn, QIcon, QTooltip, QCollapsible, clone
    },
    data: () => ({
      table,
      config: {
    ...
    methods: {
      ...
      selection (number, rows) {
        Toast.create({
          html: `Selected: ${number} : ${rows.length}`,
          icon: 'camera_enhance'
        })
      },
      rowClick (row) {
