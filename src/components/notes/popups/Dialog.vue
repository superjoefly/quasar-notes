<template>
  <div class="layout-padding">
    <p class="text-bold">Dialog</p>

    <q-list style="max-width: 600px">
      <q-list-header>Basic</q-list-header>
      ...
      <q-item link v-for="dialog in form" :key="dialog.label" @click="dialog.handler()" v-ripple.mat>
        <q-item-side :icon="dialog.icon" />
        <q-item-main :label="dialog.label" />
        <q-item-side right icon="keyboard_arrow_right" />
      </q-item>
      ...
      <q-item link v-for="position in ['top', 'bottom', 'right', 'left']" :key="position" :position="position" @click="showFromEdge(position)" v-ripple.mat>
  ...
  data () {
    return {
      types: [
        ...
        {
          label: 'Customized Buttons Example',
